...
with open('manifests/cloud-provider-config.yaml') as file:
    ...
    config.vnetName = f"{infra_id}-vnet"
    config.vnetResourceGroup = resource_group
    config.subnetName = f"{infra_id}-worker-subnet"
    config.securityGroupName = f"{infra_id}-nsg"
...
with open('manifests/cluster-infrastructure-02-config.yml') as file:
    ...
    yamlx['status']['platformStatus']['azure'][
        'virtualNetwork'
    ] = f"{infra_id}-vnet"

    yamlx['status']['platformStatus']['azure'][
        'controlPlaneSubnet'
    ] = f"{infra_id}-master-subnet"

    yamlx['status']['platformStatus']['azure'][
        'computeSubnet'
    ] = f"{infra_id}-worker-subnet"
